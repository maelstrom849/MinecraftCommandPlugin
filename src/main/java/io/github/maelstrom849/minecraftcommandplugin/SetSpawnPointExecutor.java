...
public class SetSpawnPointExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 4) {
			...
			// Make sure that the world specified by the user exists, otherwise it stops execution 
			...
			// Cast the inputs (Strings) to double. The method getDouble returns NaN if the string
			...
			// Check that none of the values of x, y, z are NaN. If they are, tell the user and
			// stop execution
			...
			// Now that all foreseen roadblocks have been passed, create a new spawn location and set 
			// it as default spawn
			Spawn.setDefaultSpawn(new Spawn(world, x, y, z));
			
			// After setting the default spawn location in the spawn class, set it for the server
			// so that new players and dead players are also affected, not just those who use the
			// /spawn command
			...
			// If there are 6 arguments they are of the form [world] [x] [y] [z] [pitch] [yaw]
			// This command can be used by players or console
			// This case is the same as if args.length == 4 with the added float arguments
			// of pitch and yaw. getFloat works the same as getDouble so they are just added in.
			World world = Bukkit.getServer().getWorld(args[0]);
			if (world == null) {
				sender.sendMessage("That world was not found.");
				return false;
			...
			if (sender instanceof Player) {
				...
				// If this command is run from the console or any other non-player entity, this is send.
